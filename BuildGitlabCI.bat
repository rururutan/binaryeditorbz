...
md build
cd build
cmake -G "Visual Studio 16 2019" -A win32 -DZLIB_LIBRARY=%CI_BUILDS_DIR%\Bz\zlib-1.2.12\build\Release\zlibstatic.lib -DZLIB_INCLUDE_DIR=%CI_BUILDS_DIR%\Bz\zlib-1.2.12 -DBUILD_SHARED_LIBS=OFF  -D CMAKE_CXX_FLAGS_RELEASE:STRING="/MT /Os /GF /GL /I\"%CI_BUILDS_DIR%\Bz\WTL91_5321_Final\Include\"" ..
cmake --build . --config Release
cd ..
cd zlib-1.2.12
md build64
...
md build64
cd build64
cmake -G "Visual Studio 16 2019" -A x64 -DZLIB_LIBRARY=%CI_BUILDS_DIR%\Bz\zlib-1.2.12\build64\Release\zlibstatic.lib -DZLIB_INCLUDE_DIR=%CI_BUILDS_DIR%\Bz\zlib-1.2.12 -DBUILD_SHARED_LIBS=OFF  -D CMAKE_CXX_FLAGS_RELEASE:STRING="/MT /Os /GF /GL /I\"%CI_BUILDS_DIR%\Bz\WTL91_5321_Final\Include\"" ..
cmake --build . --config Release
cd ..
cd nsis
